...
def get_keyboard():
    ...
    my_keyboard = ReplyKeyboardMarkup([
                                        ['Прислать фотку', 'Сменить аватарку'],
...
def main():
    ...
    dp.add_handler(RegexHandler('^(Прислать фотку)$', send_cat_picture, pass_user_data=True))

    dp.add_handler(RegexHandler('^(Сменить аватарку)$', change_avatar, pass_user_data=True))

    dp.add_handler(MessageHandler(Filters.contact, get_contact, pass_user_data=True))
    dp.add_handler(MessageHandler(Filters.location, get_contact, pass_user_data=True))
